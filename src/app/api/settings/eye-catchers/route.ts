...
const BACKEND_BASE_URL = process.env.NEXT_PUBLIC_ADMIN_API_BASE_URI
  ? `${process.env.NEXT_PUBLIC_ADMIN_API_BASE_URI}/settings/eye-catchers`
  : 'https://zotly.onrender.com/settings/eye-catchers';
  ...
  if (action === 'list') {
    try {
      ...
      return NextResponse.json(
        { message: error.message || 'Failed to fetch eye catchers' },
        { status: 500 }
      );
    }
  } else {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (text && contentType?.includes('application/json')) {
      ...
      data = { message: 'Eye catcher added successfully', ...body };
    }
    return NextResponse.json(data);
  } catch (error: any) {
    console.error('Error in POST /save:', error.message, error.stack);
    return NextResponse.json(
      { message: error.message || 'Failed to add eye catcher' },
      { status: 500 }
    );
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (text && contentType?.includes('application/json')) {
      ...
      data = { message: 'Eye catcher updated successfully', ...body };
    }
    return NextResponse.json(data);
  } catch (error: any) {
    console.error('Error in PUT /update:', error.message, error.stack);
    return NextResponse.json(
      { message: error.message || 'Failed to update eye catcher' },
      { status: 500 }
    );
  }
}
  ...
  if (action === 'clear') {
    try {
      ...
      if (text && contentType?.includes('application/json')) {
        ...
        data = { message: 'All eye catchers cleared successfully' };
      }
      return NextResponse.json(data);
    } catch (error: any) {
      console.error('Error in DELETE /clear:', error.message, error.stack);
      return NextResponse.json(
        { message: error.message || 'Failed to clear eye catchers' },
        { status: 500 }
      );
    }
  } else if (id) {
    try {
      ...
      if (text && contentType?.includes('application/json')) {
        ...
        data = { message: `Eye catcher ${id} deleted successfully` };
      }
      return NextResponse.json(data);
    } catch (error: any) {
      console.error(`Error in DELETE /delete/${id}:`, error.message, error.stack);
      return NextResponse.json(
        { message: error.message || 'Failed to delete eye catcher' },
        { status: 500 }
      );
    }
  } else {
