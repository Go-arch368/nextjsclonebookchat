...
const BACKEND_BASE_URL = process.env.NEXT_PUBLIC_ADMIN_API_BASE_URI
  ? `${process.env.NEXT_PUBLIC_ADMIN_API_BASE_URI}/settings/greetings`
  : 'https://zotly.onrender.com/settings/greetings';
  ...
  if (action === 'list') {
    try {
      ...
      return NextResponse.json(
        { message: error.message || 'Failed to fetch greetings' },
        { status: 500 }
      );
    }
  } else {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (text && contentType?.includes('application/json')) {
      ...
      data = { message: 'Greeting added successfully', ...body };
    }
    return NextResponse.json(data);
  } catch (error: any) {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (text && contentType?.includes('application/json')) {
      ...
      data = { message: 'Greeting updated successfully', ...body };
    }
    return NextResponse.json(data);
  } catch (error: any) {
    console.error('Error in PUT /update:', error.message, error.stack);
    return NextResponse.json(
      { message: error.message || 'Failed to update greeting' },
      { status: 500 }
    );
  }
}
  ...
  if (action === 'clear') {
    try {
      ...
      if (text && contentType?.includes('application/json')) {
        ...
        data = { message: 'All greetings cleared successfully' };
      }
      return NextResponse.json(data);
    } catch (error: any) {
      console.error('Error in DELETE /clear:', error.message, error.stack);
      return NextResponse.json(
        { message: error.message || 'Failed to clear greetings' },
        { status: 500 }
      );
    }
  } else if (id) {
    try {
      ...
      if (text && contentType?.includes('application/json')) {
        ...
        data = { message: `Greeting ${id} deleted successfully` };
      }
      return NextResponse.json(data);
    } catch (error: any) {
      console.error(`Error in DELETE /delete/${id}:`, error.message, error.stack);
      return NextResponse.json(
        { message: error.message || 'Failed to delete greeting' },
        { status: 500 }
      );
    }
  } else {
