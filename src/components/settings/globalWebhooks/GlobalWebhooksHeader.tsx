...
const GlobalWebhooksHeader: React.FC<GlobalWebhooksHeaderProps> = ({
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof GlobalWebhook; direction: 'asc' | 'desc' } | null>(null);
  ...
  const sortedWebhooks = useMemo(() => {
    ...
    sortableItems.sort((a, b) => {
      ...
      if (key === 'dataTypeEnabled') {
        aValue = a.dataTypeEnabled ? 'Enabled' : 'Disabled';
        bValue = b.dataTypeEnabled ? 'Enabled' : 'Disabled';
      } else {
        aValue = String(a[key] ?? '');
        bValue = String(b[key] ?? '');
      }
  ...
  const filteredWebhooks = sortedWebhooks.filter(
    (webhook) =>
      ...
      webhook.destination.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const fetchWebhooks = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await axios.get<GlobalWebhook[]>(`${API_BASE_URL}?action=list`);
      ...
      const message =
        err.response?.status === 404
          ...
          : err.response?.data?.message || err.message || 'Failed to fetch global webhooks';
        ...
        const handleSearch = async () => {
          try {
            ...
            const message =
              err.response?.status === 404
                ? 'Global Webhooks API route not found. Please check the server configuration.'
                : err.response?.data?.message || err.message || 'Failed to search global webhooks';
  ...
  const handleSort = (key: keyof GlobalWebhook) => {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      const message =
        err.response?.status === 404
          ? 'Global Webhooks API route not found. Please check the server configuration.'
          : err.response?.data?.message || err.message || 'Failed to delete global webhook';
  ...
  const handleDeleteSelected = async () => {
    if (selectedRows.length === 0) {
      toast.warn('No global webhooks selected for deletion.', {
        position: 'top-right',
        autoClose: 3000,
      });
      return;
    ...
    try {
      ...
      toast.success('Selected global webhooks deleted successfully!', {
        position: 'top-right',
        autoClose: 3000,
      });
    } catch (err: any) {
      const message =
        err.response?.status === 404
          ? 'Global Webhooks API route not found. Please check the server configuration.'
          : err.response?.data?.message || err.message || 'Failed to delete selected global webhooks';
  ...
  const handleClearAll = async () => {
    try {
      ...
      setWebhooks([]);
      setCurrentPage(1);
      toast.success('All global webhooks cleared successfully!', {
        position: 'top-right',
        autoClose: 3000,
      });
    } catch (err: any) {
      const message =
        err.response?.status === 404
          ? 'Global Webhooks API route not found. Please check the server configuration.'
          : err.response?.data?.message || err.message || 'Failed to clear global webhooks';
          ...
          <Table className="border border-gray-200 w-full">
            <TableHeader>
              <TableRow>
                ...
                {['event', 'email', 'targetUrl', 'dataTypeEnabled', 'destination', 'createdBy'].map((col) => (
                  <TableHead key={col} className="px-4 py-4 hover:bg-gray-100 w-[15%] text-center">
                    <Button
                      variant="ghost"
                      onClick={() => handleSort(col as keyof GlobalWebhook)}
                ...
                <TableHead className="px-4 py-4 hover:bg-gray-100 w-[15%] text-center">Details</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {currentWebhooks.map((item) => (
                <TableRow key={item.id} className="hover:bg-gray-100">
                  ...
                  <TableCell className="px-4 py-3 w-[15%] text-left truncate">{item.event}</TableCell>
                  <TableCell className="px-4 py-3 w-[15%] text-left truncate">{item.email}</TableCell>
                  <TableCell className="px-4 py-3 w-[15%] text-left truncate">{item.targetUrl}</TableCell>
                  <TableCell className="px-4 py-3 w-[15%] text-center">
                    {item.dataTypeEnabled ? 'Enabled' : 'Disabled'}
                  </TableCell>
                  <TableCell className="px-4 py-3 w-[15%] text-center">{item.destination}</TableCell>
                  <TableCell className="px-4 py-3 w-[15%] text-center">
